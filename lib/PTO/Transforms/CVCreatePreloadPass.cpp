...
// MapVector keeps insertion order so the IR generated by rewritePreloadLoop is
// stable across runs. A plain DenseMap iterates in pointer-hash order, which
// makes the cloned per-stage pointer_cast / bind_tile sequence non-deterministic
// and shifts SSA numbering on every build (P0-1 fix).
using LocalBufferBindingMap = llvm::MapVector<Value, LocalBufferBinding>;

static LogicalResult collectLocalBufferBindings(
    CVScopeOp scope, int64_t maxPreloadNum, scf::ForOp forOp,
    LocalBufferBindingMap &bindings) {
...
static LogicalResult rewritePreloadLoop(IRRewriter &rewriter, scf::ForOp forOp,
  ...
  LocalBufferBindingMap localBindings;
  ...
  // MapVector preserves insertion order; iterate via .takeVector() equivalent
  // (range-for over the MapVector visits in insertion order, not pointer-hash
  // order). The SSA values cloned per stage thus appear in a stable sequence.
  ...
  // P1-7: Hoist per-stage pointer_cast / bind_tile clones to *above* the new
  // for op. The original addrs are loop-invariant (PlanMemory emits constant
  // i64 offsets), so the rotated multi-address casts are loop-invariant too.
  // Inserting them inside the new for body makes the subsequent
  // pto-enable-multi-buffer pass see a multi-address pointer_cast inside a
  // loop, which then re-hoists N single-address casts per stage; with M stages
  // and N slots that is M*N copies instead of N. Hoisting once here keeps the
  // rotation deterministic and gives the next pass a single-address-per-stage
  // shape to work with.
  ...
  for (int64_t preloadNum = 0; preloadNum < maxPreloadNum; ++preloadNum) {
    for (LocalBufferBinding &binding : orderedBindings) {
      clonePointerCastAndBind(rewriter, binding, maxPreloadNum, preloadNum,
  ...
  // Stage IVs depend on the new induction variable and so must live inside
  // the new for body. Wire them into the per-stage mappings used by clones.
