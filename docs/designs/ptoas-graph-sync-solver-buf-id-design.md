...
5. **裁剪阶段**（`checkGraphConflict`）：
   ...
   - 同 pipe-pair 不同 consumer 的现有 pair：若候选 consumer 写了与现有
     ...
     ratchet，候选可被裁剪。
   - 同 pipe-pair 不同 producer 的现有 pair：不能传递覆盖（与上一条对称——
     buf-id 没有 producer-side FIFO 自动覆盖语义，每个 producer 都需要自己
     的 bracket 进入 chain；尤其要保证 cross-iter 时 first producer 也在
     ratchet 链上）。
...
4. **同 pipe-pair 但不同 producer 的现有 pair**（与规则 3 对称）：
   `conflictPair->op1 != candOp1 && conflictPair->op2 == candOp2` 且 pipe-pair /
   conflictBuffer 都匹配 → 丢弃。

   理由：set/wait 模式下，同 producer pipe FIFO 让最晚的 producer 一次
   `set_flag` 覆盖更早的所有 producer——求解器的传递覆盖正是基于这条
   FIFO 性质。buf-id 没有这条性质：每个 producer 的 bracket 锚在自己
   的 op 上，counter ratchet 只串联**有 bracket 的**锚点。让现有 pair
   传递覆盖，会把候选 producer 排除在 chain 之外。

   典型受影响场景：循环里多个 matmul producer (writes `v46`) 喂同一个
   `tstore` consumer，外层循环跨迭代用 `v46` 形成 ping-pong 累加。
   若只保留 last producer 的 bracket，iter k+1 的 first matmul（同
   OperationBase 多次 Occurrence）在 PIPE_M FIFO 上仅等 iter k 的
   last matmul 完成，**不**等 iter k 的 tstore 完成 → iter k+1 的
   accumulator 写覆盖 iter k 仍在读的 v46，data race。规则 4 保证所有
   producer 的 ConflictPair 都不会被传递裁剪，进入候选集合。

5. 其余 pair 走原有 Dijkstra 覆盖逻辑。

#### 3.8.3.1 Middle-producer 二次 emit 裁剪

规则 4 让所有 producer 都拿到 ConflictPair，但事实上 chain 的中间 producer
不需要 bracket——它们与 first / last 都在同一 pipe，A5 PIPE_M FIFO 已经把
它们串行化。emit 阶段额外做一遍裁剪：

> 对每个 forward pair P，扫描 `forwardPairs` 中所有 (op2, conflictBuffer,
> setPipe, waitPipe) 都匹配但 op1 不同、且**不与 P 互斥**的 peer Q。按
> `opAllOccurrences[P->op1].front()->syncIrIndex` 对比位置：
> - 若存在 earlier peer（`Q.idx < P.idx`）且 also 存在 later peer
>   (`Q.idx > P.idx`)：P 是某条非互斥 path 的中间 producer → emit 阶段
>   跳过 P。
> - 否则 P 是 path 中的 first 或 last（或 path 唯一成员）→ emit。

互斥 peer 不计入对比，因为运行时 path 不同（同一时刻只走一条 path）。
使用 `op1` 的 `opAllOccurrences.front()->syncIrIndex` 而不是 `cp->startIndex`
是因为后者是 setOp（被 LCA hoist 过的 PlaceHolder）的位置，多个不同 op1
会塌缩到同一边界点，丢失相对顺序。

gemm.pto 外层 if-branch 的 matmul 链举例：
| matmul | mutex 路径成员 | earlier peer? | later peer? | emit? |
| --- | --- | --- | --- | --- |
| inner_if_TMATMUL | {inner_if_TMATMUL, _2nd, _3rd, _4th} | 无 | 有 (_2nd 等) | ✓ first |
| inner_else_TMATMUL_ACC | {inner_else, _2nd, _3rd, _4th} | 无 | 有 | ✓ first |
| _2nd | 与 inner_if / inner_else 不互斥（在 inner 之后） | 有 | 有 | ✗ middle |
| _3rd | 同上 | 有 | 有 | ✗ middle |
| _4th | 同上 | 有 | 无 | ✓ last |

else-branch 同理：_1st emit，_2nd / _3rd skip，_4th emit。
...
**例子 4：multiple producers, one consumer across outer loop（gemm.pto
accumulator + tstore）**
```text
for v53 {
  for v55 {
    matmul_1st (writes v46)         # in inner if/else
    matmul_ACC_2nd (reads/writes v46)
    matmul_ACC_3rd (reads/writes v46)
    matmul_ACC_4th (reads/writes v46)
  }
  tstore v66, v46                   # reads v46, in v53 body
}
```
- 4+ 个 forward pair `(matmul_X, tstore)`，全部在 `v46` 上，全部
  PIPE_M → PIPE_FIX。
- 候选 `(matmul_1st, tstore)`，现有 `(matmul_ACC_4th, tstore)`：
  - 同 conflictBuffer (v46)、同 pipe-pair (M → FIX)、同 op2 (tstore)、
    不同 op1 → 命中 §3.8.3 规则 4 → 现有 pair 丢弃 → 候选自己进入
    ConflictPair 集合。
  - 中间 producer (_2nd / _3rd) 也按规则 4 保留 ConflictPair。
- emit 阶段（§3.8.3.1）扫描 path 内的 earlier / later peer：
  - matmul_1st 是 path first → emit `(M, id_v46)` bracket。
  - matmul_ACC_4th 是 path last → emit。
  - _2nd / _3rd 是 middle → 跳过 emit；A5 PIPE_M FIFO 自动 ordering。
- 结果：每个 path 上只有 first 和 last 的 matmul 在 PIPE_M 上有
  id_v46 的 get/rls bracket，counter 按 first → tstore → next-iter first
  跨 outer 迭代 ratchet。
- 跨外层迭代：iter k+1 的 matmul_1st（same OperationBase, 不同 Occurrence）
  get_buf 时 snap 已经在 iter k 的 tstore rls 之后，硬件等到 iter k
  tstore 读完 v46 才允许 iter k+1 写 v46，WAR 安全。
...
| S9 | `checkGraphConflict` 同 pipe-pair 不同 producer 的对称裁剪规则（避免 cross-iter 累加器 WAR 漏掉 first producer） | ✅ 已落地（commit `880ac9f7`） |
| S10 | Emit-阶段 middle-producer 裁剪（只 emit path first / last，靠 PIPE_M FIFO 串联 middle） | ✅ 已落地 |
| S11 | 更新 `PTO_IR_manual.md` / `ptoas-auto-sync-design.md` | TODO |
